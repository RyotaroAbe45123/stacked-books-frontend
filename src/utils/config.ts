export const url = "https://stacked-books.ml"

export const hashTags = "積読,StackedBooks"

export const tweetUrl = "https://twitter.com/intent/tweet"

export const timeout: number = 30 * 1000;

export const pageSize: number = 8;

export const openbdEndpoint = "https://api.openbd.jp/v1/get";

export const openbdBookImageEndpoint = "https://cover.openbd.jp/";

export const issBookImageEndpoint: string = "https://iss.ndl.go.jp/thumbnail/";

export const mobileMaxWidth = 450;